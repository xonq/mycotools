...
def read_predb(predb_path, spacer="\t"):
    ...
    allowed2required = {
        ...
        "genomeSource": "genomeSource (ncbi/jgi/new)",
    }
    ...
    if not "gffPath" in predb:
        if "gff3_path" in predb:
            ...
            raise KeyError
    try:
        if "restriction" not in predb and "useRestriction (yes/no)" in predb:
...
def predb2mtdb(predb):
    ...
    if not "previous_ome" in predb and "ome" in predb:
        ...
        predb["previous_ome"] = ["" for x in predb[list(predb.keys())[0]]]
...
def gen_omes(newdb, refdb=None, ome_col="ome", forbidden=set(), spacer="\t"):
    ...
    if ref_ome_check:
        raise ValueError(
            "corrupted reference database with non-unique omes: " + str(ref_ome_check)
        )
    #   elif new_ome_check:
    ...
    refdb_nover = {re.sub(r"(^.{6}\d+)\.\d+", r"\1", x): x for x in list(refdb["ome"])}
        ...
        if not ome:  # if there isn't an ome for this entry yet
            ...
            try:
                ...
                if not isinstance(newdb["assembly_acc"][i], float):
                    ...
                    # info
                    continue
                row = {key: newdb[key][i] for key in mtdb.columns if key != "ome"}
...
def gff_mngr(ome, gff, cur_path, source, assembly_accession):
    ...
    for line in gff:
        ...
        if seqid.startswith(less_ome + "_"):
            line["seqid"] = re.sub(r"^" + less_ome + "_", ome + "_", seqid)
        elif re.search(r"^" + less_ome + r"\.\d+_", seqid):
            line["seqid"] = re.sub(r"^" + less_ome + r"[^_]+_", ome + "_", seqid)
        else:
...
def cli():
    ...
    omedb, failed = main(
        predb, refDB, wrk_dir, exit=exit, verbose=True, forbidden=forbid_omes
    )
