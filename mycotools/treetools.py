...
def cli():
    parser = argparse.ArgumentParser(description="Phylogeny manipulation tools")
    parser.add_argument("-i", "--input", help="Input phylogeny", required=True)
    parser.add_argument("-t", "--tips", help="Tips to reference")
    parser.add_argument("-d", "--mtdb", help="MTDB to reference")
    parser.add_argument(
        "-p", "--prune", action="store_true", help="Remove omes absent from input"
    )
