...
def intron2exon(gff, gene_comp=re.compile(r"gene_id \"(.*?)\"")):
    ...
    for entry in gff:
        try:
            ...
            if (
                entry["type"] in {"transcript", "gene"}
                and entry["source"] == "AUGUSTUS"
            ):
                continue
    ...
    cds_no_exon = {
        gene: {"cds": info["cds"], "raw": info["raw"]}
    ...
    for gene in intron_genes:
        ...
        if intron_genes[gene]["strand"] == "+":
            if intron_genes[gene]["start_codon"]:
                ...
                exon_coords[gene].append(
                    [min([int(x["start"]) for x in intron_genes[gene]["raw"]])]
                )
            ...
            if intron_genes[gene]["stop_codon"]:
                ...
                exon_coords[gene][-1].append(
                    max([int(x["end"]) for x in intron_genes[gene]["raw"]])
                )
                            ...
                            list(
                                chain(
                                    *[
                                        ...
                                        for x in intron_genes[gene]["raw"]
                                    ]
                                )
                            )
                        )
    ...
    for gene in exon_coords:
        try:
            add_data = [
                entry
                for entry in intron_genes[gene]["raw"]
                if entry["type"] != "intron"
            ]
...
def curCDS(gff, gene_compile=re.compile(r"gene_id \"(.*?)\"")):
    ...
    for entry in gff:
        ...
        if entry["type"] == "gene":
            ...
            info_dict[gene]["start_codon"].extend(
                sorted([int(entry["end"]), int(entry["start"])])
            )
            info_dict[gene]["stop_codon"].extend(
                sorted([int(entry["end"]), int(entry["start"])])
            )
        elif entry["type"] in {"start_codon", "stop_codon"}:
            info_dict[gene][entry["type"]].extend(
...
def liberalRemoval(gene_dict_prep, contigs):
    ...
    for gene in gene_dict_prep:
        if gene_dict_prep[gene]["start_codon"] and gene_dict_prep[gene]["stop_codon"]:
            ...
            if (
                not gene_dict_prep[gene]["start_codon"]
                or not gene_dict_prep[gene]["stop_codon"]
            ):
...
def curate(gff, prefix=None, failed=set(), comps=gtfComps()):
    ...
    for line in gff:
        if (
            line["type"] in {"exon", "start_codon", "stop_codon", "CDS"}
            or "RNA" in line["type"]
        ):
            try:
            ...
            temp_exon_dict[line["seqid"]][tran_id].extend(
                [int(line["start"]), int(line["end"])]
            )
    ...
    for entry in crudesortGff:
        ...
        if "RNA" in entry["type"]:
            ...
            entry["attributes"] = f"ID={trans}.exon{exon};Parent=" + f"{trans}"
            if prefix:
                entry["attributes"] += f";Alias={alias_dict[trans]}"
            exon_check[trans] += 1
        elif entry["type"] == "CDS":
...
def cli():
    ...
    parser = argparse.ArgumentParser(
        description="Curates Funannotate or "
    ...
    parser.add_argument(
        "-a", "--assembly", help="Assembly fasta for proteome export. Requires -p"
    )
    parser.add_argument("-p", "--prefix", help="Ome code for alias")
    parser.add_argument(
        "-s", "--sort", action="store_true", help="Only sort compatible gff3"
    )
