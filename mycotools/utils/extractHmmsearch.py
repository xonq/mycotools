...
def sort_hits(hit_list, aln_list):
    ...
    sortd_hits = {
        i: v
        for i, v in sorted(hit_dict.items(), key=lambda x: float(x[1][2]), reverse=True)
    }
...
def grab_hits(
    ...
    if hitsAligns is not None:
        ...
        for line in outputLines:
            hit_str += (
                f"{' '.join(line[8:])}\t{line[0]}\t{line[1]}\t{line[2]}\t"
...
def main(data, accession, best, threshold, evalue, bitscore, query=True, header=True):
    ...
    if isinstance(check, set):
        for x in check:
            ...
            if header:
                ...
                align = (
                    "#seq\tdomNum\tincl_thresh\tscore\tbias\tc-Evalue\ti-Evalue\thmmFrom\t"
                    + "hmmTo\taliFrom\taliTo\tenvFrom\tenvTo\tacc\n"
                )
            if accession:
                t_hits, t_aligns = grab_hits(
                    data, threshold, evalue=evalue, top=best, accession=x
                )
            else:
                t_hits, t_aligns = grab_hits(
                    data, threshold, evalue=evalue, top=best, query=x
                )
            if t_hits:
                hits += t_hits
                align += t_aligns
        ...
        t_hits, t_aligns = grab_hits(
            data, threshold, top=best, query=query, accession=accession
        )
        hits += t_hits
        align += t_aligns
        if accession:
...
def cli():
    ...
    parser = argparse.ArgumentParser(
        description="Extracts inputted accession hits and alignments from `hmmsearch` "
    ...
    parser.add_argument(
        ...
        "--threshold",
        type=float,
        help="Percent query coverage to consider a hit (float).",
    )
    parser.add_argument(
        "-e",
        "--evalue",
        help="Negative magnitude of E-value, e.g. " + "10^(-x) where `x` is the input.",
        type=float,
        default=0,
    )
