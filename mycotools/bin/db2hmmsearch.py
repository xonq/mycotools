...
from mycotools.bin.extractHmmsearch import main as exHmm
from mycotools.bin.acc2fa import main as acc2fa
from mycotools.bin.abstractHmm import grabAccs, main as absHmm
...
#def main( args, db, output, cpu, previous = False ):
    ...
    if previous:
        ...
        if not skip1:
            ...
            print('\thmmsearch -> hits.fa DONE')
            if skip2:
                print('\thits.fa -> phylip DONE')

    if not skip1:
        print( '\nSubmitting omes to hmmsearch' )
        if not os.path.isdir( output + '/omes' ):
        ...
        for code in hmmsearch_codes:
            if code['code'] != 0:
                print( '\tExit ' + str(code['code']) + ' ' + \
                     os.path.basename( code['stdin'][-1] ) )
    
        print( '\nExtracting hmmsearch output' )
    ...
    if args.align:
        if not skip2:
            ...
            for index in range( len (hmmAlign_codes) ):
                ...
                if code['code'] != 0:
                    print( '\thmmalign exit ' + str(code['code']) + ' ' + \
                         os.path.basename( code['stdin'][-1] ) )
                else:
                    code = esl_codes[ index ]
                    if code['code'] != 0:
                        print( '\tesl-reformat exit ' + str(code['code']) + ' ' + \
                            os.path.basename( code['stdin'][-1] ) )
