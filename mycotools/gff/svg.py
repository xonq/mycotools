...
def compile_products(gff, prod_comp, types={"tRNA", "mRNA", "rRNA"}):
    ...
    out_products = list(set([x.replace('"', "") for x in products]))
    return out_products
    ...
    for seq in seqs:
        ...
        for i in seqs[seq]:
            ...
            if i[-1] == null or not labels:
                features.append(
                    GraphicFeature(start=i[0], end=i[1], strand=i[2], color=i[3])
                )
            else:
                features.append(
                    GraphicFeature(
                        start=i[0], end=i[1], strand=i[2], color=i[3], label=i[4]
                    )
                )
...
def cli():
    ...
    parser.add_argument(
        "-w", "--width", type=int, default=10, help=".svg width; DEFAULT: 10"
    )
    parser.add_argument(
        "-r",
        "--regex",
