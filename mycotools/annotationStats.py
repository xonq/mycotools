...
def compileExon( gff_path, output ):
    ...
    if not output:
        print( '{:<25}'.format('GENE LENGTH:') + str(total) )
        print( '{:<25}'.format('GENES:') + str(len(exon_dict)))
        print( '{:<25}'.format('MEAN GENE LENGTH:') + str(total/len(exon_dict)))
        print( '{:<25}'.format('MEDIAN GENE LENGTH:') + str(median))
        geneStats = None
    else:
