...
def start_check(osc_number, remote=1, server='owens.osc.edu', username='osu10393', pw='', spacer = '\t'):

#HARD CODE
# generate command to check job status and grep job number
    check_cmd = "/opt/torque/bin/qstat -u " + username + " | grep " + osc_number
    count = 0
# if remote is set to no, execute the command locally
    if remote is 0:
        status = subprocess.run(check_cmd, shell=True, stdout = subprocess.PIPE)
        status_byte = status.stdout.rstrip()
        status = status_byte.decode('utf-8')

# otherwise, open up an SSH connection, default OSC
    else:
        while True:
            try:
...
# in the output 
    if status != '?':
        if re.search(r' Q.*--',status[-15:]):
            start_code = 'Q'
        elif re.search(r' C.*--',status[-15:]):
            start_code = 'C'
        elif re.search(r' R\W*.*',status[-15:]):
            start_code = 'R'
        else:
            eprint(spacer + 'ERROR:\t' + osc_number + ' not in `qstat` - fast execution/error')
            start_code = '?'
...
def execution(prog_statement, osc_param_dict, remote=1, server='owens.osc.edu', username='osu10393', pw='', spacer='\t', start_cmd = 1, interval = 1, verbose = 0):
...
# add the program statement to the osc param and make cmd
    ...
    exec_statement = 'echo -e "{prog_run}" | /opt/torque/bin/qsub -l walltime={walltime} -l nodes={nodes}:ppn={threads} -j oe -N {log_name} -A {project} -o {output_dir}'.format(**osc_param_dict)
    if verbose == 1:
        print(spacer + exec_statement)


# if it is not remote, submit via subprocess, gather job
    if remote == 0:
        osc_submit = subprocess.run(exec_statement,shell=True,stdout=subprocess.PIPE,stderr=subprocess.PIPE,check=True)
        osc_stdout_byte = osc_submit.stdout.rstrip()
        osc_stdout = osc_stdout_byte.decode('utf-8')
        job_number = re.match(r'\d+',osc_stdout)[0]

# otherwise, open an ssh client, submit, grab job no
    else:
...
# NEED to check for errors here
        job_number = re.match(r'\d+',stdout_read[0])[0]

    log_path = output_var + '/' + log_var + '.o' + job_number
...
def status_check(output_dir,log_path,pw='',server='owens.osc.edu',username='osu10393',error_exit=1,spacer='\t', port = 22):
...
# grab the log file and save in current dir
    ...
    # if the log file has resource units charged, it is done
        if re.search(r'Resources used:\ncput',log_data):
#            print('\n\n' + spacer + 'Execution finished:',os.path.basename(os.path.splitext(log_file)[0]))
            exit_code = 'C'
